...
serve(async (req) => {
  ...
  try {
    ...
    if (!isMarketOpen) {
      console.log(`[paper-runner] Market closed. ET time: ${etTime.toISOString()}`);
      return new Response(JSON.stringify({ 
        success: true, 
    ...
    console.log(`[paper-runner] Template: ${templateId}, Params:`, params);
    ...
    const symbol = symbols[0]; // Focus on first symbol for now
    ...
    const atrMultiplierStop = params.atr_multiplier_stop || 1.5;
    const atrMultiplierTarget = params.atr_multiplier_target || 2.0;
    ...
    if (signal.type === 'entry_long' && !currentPosition) {
      ...
      if (qty > 0) {
        const stopLoss = latestBar.c - (atr * atrMultiplierStop);
        const takeProfit = latestBar.c + (atr * atrMultiplierTarget);
...
function generateSignal(
  ...
  if (templateId === 'momentum_breakout_v1') {
    ...
    const breakoutThreshold = params.breakout_threshold || 0.02;
    ...
    // Check for breakout
    const upperBreakout = recentHigh * (1 + breakoutThreshold);
    const lowerBreakout = recentLow * (1 - breakoutThreshold);
    
    if (!currentPosition) {
      if (price > upperBreakout) {
        return { type: 'entry_long', reason: `breakout_above_${recentHigh.toFixed(2)}` };
      }
      if (price < lowerBreakout) {
